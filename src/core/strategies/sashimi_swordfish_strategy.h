...
/// Strategy for finding Sashimi Swordfish patterns.
/// Like a Finned Swordfish, but at least one base row (or column) has only 1 candidate position
/// instead of the minimum 2 required by finned fish. The "missing" position is what makes it sashimi.
/// The union of columns is 4 (N+1 for order-3 fish). Eliminations are restricted to the 3 base columns
/// in cells that also share the fin's box.
class SashimiSwordfishStrategy : public ISolvingStrategy, protected StrategyBase {
public:
    ...
    [[nodiscard]] SolvingTechnique getTechnique() const override {
        return SolvingTechnique::SashimiSwordfish;
    }
    ...
    [[nodiscard]] double getDifficultyRating() const override {
        return getTechniqueRating(SolvingTechnique::SashimiSwordfish);
    }
...
private:
    ...
    [[nodiscard]] static std::optional<SolveStep> findRowBased(const BoardData& board,
        ...
        for (int value = MIN_VALUE; value <= MAX_VALUE; ++value) {
            ...
            for (size_t r1 = 0; r1 < BOARD_SIZE; ++r1) {
                if (rows_cols[r1].empty() || rows_cols[r1].size() > 4) {
                    continue;
                }
                for (size_t r2 = r1 + 1; r2 < BOARD_SIZE; ++r2) {
                    if (rows_cols[r2].empty() || rows_cols[r2].size() > 4) {
                        continue;
                    }
                    for (size_t r3 = r2 + 1; r3 < BOARD_SIZE; ++r3) {
                        if (rows_cols[r3].empty() || rows_cols[r3].size() > 4) {
                            continue;
                        }

                        // At least one row must have exactly 1 position (sashimi condition)
                        bool has_sashimi =
                            (rows_cols[r1].size() == 1 || rows_cols[r2].size() == 1 || rows_cols[r3].size() == 1);
                        if (!has_sashimi) {
                            continue;
                        }
                        ...
                        auto result =
                            tryRowSashimiSwordfish(board, candidates, value, rows_cols, r1, r2, r3, union_cols);
                        if (result.has_value()) {
                            return result;
                        }
                    }
    ...
    tryRowSashimiSwordfish(const BoardData& board, const CandidateGrid& candidates, int value,
        ...
        for (size_t fi = 0; fi < 4; ++fi) {
            ...
            auto explanation = fmt::format("Sashimi Swordfish on value {} in Rows {}, {}, {} with fin at {} — "
            ...
            return SolveStep{.type = SolveStepType::Elimination,
                             .technique = SolvingTechnique::SashimiSwordfish,
                             .position = eliminations[0].position,
                             .value = 0,
                             .eliminations = eliminations,
                             .explanation = explanation,
                             .rating = getTechniqueRating(SolvingTechnique::SashimiSwordfish),
    ...
    [[nodiscard]] static std::optional<SolveStep> findColBased(const BoardData& board,
        ...
        for (int value = MIN_VALUE; value <= MAX_VALUE; ++value) {
            ...
            for (size_t c1 = 0; c1 < BOARD_SIZE; ++c1) {
                if (cols_rows[c1].empty() || cols_rows[c1].size() > 4) {
                    continue;
                }
                for (size_t c2 = c1 + 1; c2 < BOARD_SIZE; ++c2) {
                    if (cols_rows[c2].empty() || cols_rows[c2].size() > 4) {
                        continue;
                    }
                    for (size_t c3 = c2 + 1; c3 < BOARD_SIZE; ++c3) {
                        if (cols_rows[c3].empty() || cols_rows[c3].size() > 4) {
                            continue;
                        }

                        bool has_sashimi =
                            (cols_rows[c1].size() == 1 || cols_rows[c2].size() == 1 || cols_rows[c3].size() == 1);
                        if (!has_sashimi) {
                            continue;
                        }
                        ...
                        auto result =
                            tryColSashimiSwordfish(board, candidates, value, cols_rows, c1, c2, c3, union_rows);
                        if (result.has_value()) {
                            return result;
                        }
                    }
    ...
    [[nodiscard]] static std::optional<SolveStep>
    tryColSashimiSwordfish(const BoardData& board, const CandidateGrid& candidates, int value,
        ...
        for (size_t fi = 0; fi < 4; ++fi) {
            ...
            auto explanation = fmt::format("Sashimi Swordfish on value {} in Columns {}, {}, {} with fin at {} — "
                                           "eliminates {} from cells in fin's box",
