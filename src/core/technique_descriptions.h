    ...
    switch (technique) {
        case NakedSingle:
            return {
                ...
                .what_it_is = core::loc("A cell has only one possible candidate left. All other values are eliminated "
        ...
        case HiddenSingle:
            return {.title = getLocalizedTechniqueName(technique),
                    .what_it_is =
                        core::loc("A value can only go in one cell within a row, column, or box. Even though the cell "
                                  "may have multiple candidates, only this value has no other place in the region."),
                    .what_to_look_for = core::loc("For each region, check if any value has only one possible cell.")};
        case NakedPair:
            return {.title = getLocalizedTechniqueName(technique),
                    .what_it_is = core::loc(
                    ...
                    .what_to_look_for =
                        core::loc("Find two cells in a row, column, or box that share the same pair of candidates.")};
        case NakedTriple:
            return {.title = getLocalizedTechniqueName(technique),
                    .what_it_is = core::loc(
        ...
        case HiddenPair:
            return {.title = getLocalizedTechniqueName(technique),
                    .what_it_is = core::loc("Two values in a region appear as candidates in exactly the same two "
                                            "cells. Other candidates in those two cells can be eliminated."),
                    .what_to_look_for =
                        core::loc("For each region, find two values that appear only in the same two cells.")};
        case HiddenTriple:
            return {.title = getLocalizedTechniqueName(technique),
                    .what_it_is = core::loc("Three values in a region appear as candidates in exactly three cells. "
                                            "Other candidates in those cells can be eliminated."),
                    .what_to_look_for =
                        core::loc("For each region, find three values confined to exactly three cells.")};
        case PointingPair:
            return {.title = getLocalizedTechniqueName(technique),
                    .what_it_is =
                        core::loc("A candidate in a box is confined to a single row or column. That candidate can be "
                                  "eliminated from the rest of that row or column outside the box."),
                    .what_to_look_for =
                        core::loc("In each box, check if a candidate appears only in one row or one column.")};
        case BoxLineReduction:
            return {
                .title = getLocalizedTechniqueName(technique),
                .what_it_is = core::loc("A candidate in a row or column is confined to a single box. That candidate "
                                        "can be eliminated from the rest of the box outside that row or column."),
                .what_to_look_for = core::loc("In each row/column, check if a candidate appears only within one box.")};
        case NakedQuad:
            return {.title = getLocalizedTechniqueName(technique),
                    .what_it_is = core::loc("Four cells in a region collectively contain exactly four candidates. "
        ...
        case HiddenQuad:
            return {.title = getLocalizedTechniqueName(technique),
                    .what_it_is = core::loc("Four values in a region appear as candidates in exactly four cells. Other "
                                            "candidates in those cells can be eliminated."),
                    .what_to_look_for = core::loc("For each region, find four values confined to exactly four cells.")};
        case XWing:
            return {.title = getLocalizedTechniqueName(technique),
                    .what_it_is = core::loc(
                    ...
                    .what_to_look_for =
                        core::loc("Find a candidate forming a rectangle pattern: two rows, two columns, four cells.")};
        case XYWing:
            return {.title = getLocalizedTechniqueName(technique),
                    .what_it_is =
                        core::loc("A pivot cell with candidates {A,B} sees two wing cells: one with {A,C} and one with "
        ...
        case Swordfish:
            return {.title = getLocalizedTechniqueName(technique),
                    ...
                    .what_to_look_for = core::loc("Extend the X-Wing pattern to three rows and three columns.")};
        case Skyscraper:
            return {.title = getLocalizedTechniqueName(technique),
                    .what_it_is =
                        core::loc("Two conjugate pairs for a digit share one endpoint in the same row or column. The "
                                  "digit can be eliminated from cells that see both non-shared endpoints."),
                    .what_to_look_for = core::loc("Find two rows (or columns) each with exactly two cells for a digit, "
        ...
        case TwoStringKite:
            return {.title = getLocalizedTechniqueName(technique),
                    ...
                    .what_to_look_for =
                        core::loc("Find a row pair and column pair for the same digit connected via a shared box.")};
        case XYZWing:
            return {
                .title = getLocalizedTechniqueName(technique),
                .what_it_is = core::loc("A pivot cell with candidates {A,B,C} sees a wing with {A,B} and a wing with "
        ...
        case UniqueRectangle:
            return {.title = getLocalizedTechniqueName(technique),
                    .what_it_is = core::loc("Four cells forming a rectangle across two boxes would create a deadly "
                    ...
                    .what_to_look_for =
                        core::loc("Find four cells in a rectangle across two boxes sharing the same two candidates.")};
        case WWing:
            return {.title = getLocalizedTechniqueName(technique),
                    .what_it_is =
                        core::loc("Two cells with the same pair of candidates {A,B} are connected by a strong link on "
        ...
        case FinnedXWing:
            return {.title = getLocalizedTechniqueName(technique),
                    ...
                    .what_to_look_for =
                        core::loc("Find an X-Wing where one row has an extra candidate cell in the same box.")};
        case RemotePairs:
            return {.title = getLocalizedTechniqueName(technique),
                    .what_it_is = core::loc(
                    ...
                    .what_to_look_for =
                        core::loc("Find a chain of identical bivalue cells connected through shared regions.")};
        case BUG:
            return {.title = getLocalizedTechniqueName(technique),
                    .what_it_is = core::loc("If all unsolved cells have exactly two candidates except one cell with "
        ...
        case Jellyfish:
            return {.title = getLocalizedTechniqueName(technique),
                    .what_it_is = core::loc(
                        ...
                        "columns. The candidate can be eliminated from other cells in those columns."),
                    .what_to_look_for = core::loc("Extend the Swordfish pattern to four rows and four columns.")};
        case FinnedSwordfish:
            return {.title = getLocalizedTechniqueName(technique),
                    .what_it_is =
                        core::loc("A Swordfish pattern with extra candidate cells (fins) in the same box. Eliminations "
                                  "are restricted to cells seeing both the fin box and the Swordfish columns."),
                    .what_to_look_for =
                        core::loc("Find a Swordfish where one row has extra candidates in the same box.")};
        case EmptyRectangle:
            return {
                .title = getLocalizedTechniqueName(technique),
                .what_it_is = core::loc(
        ...
        case FinnedJellyfish:
            return {
                .title = getLocalizedTechniqueName(technique),
                .what_it_is = core::loc("A Jellyfish pattern with extra fin cells in the same box. Eliminations are "
                                        "restricted to cells seeing both the fin box and the Jellyfish columns."),
                .what_to_look_for = core::loc("Find a Jellyfish where one row has extra candidates forming a fin.")};
        case XYChain:
            return {.title = getLocalizedTechniqueName(technique),
                    .what_it_is = core::loc("A chain of bivalue cells where consecutive cells share a candidate value, "
        ...
        case MultiColoring:
            return {.title = getLocalizedTechniqueName(technique),
                    .what_it_is =
                        core::loc("Build separate conjugate pair chains (clusters) for a digit and color each. When "
        ...
        case SueDeCoq:
            return {.title = getLocalizedTechniqueName(technique),
                    ...
                    .what_to_look_for =
                        core::loc("Find an intersection where candidates can be partitioned into two covering ALS.")};
        case ForcingChain:
            return {.title = getLocalizedTechniqueName(technique),
                    .what_it_is = core::loc("For a cell with 2-3 candidates, assume each candidate is true and "
        ...
        case NiceLoop:
            return {.title = getLocalizedTechniqueName(technique),
                    .what_it_is = core::loc("Build a chain of alternating strong and weak links between (cell, digit) "
        ...
        case ThreeDMedusa:
            return {.title = getLocalizedTechniqueName(technique),
                    .what_it_is =
                        core::loc("Multi-digit coloring: build a graph of (cell, digit) pairs connected by strong "
                    ...
                    .what_to_look_for =
                        core::loc("Extend single-digit coloring to multiple digits via bivalue cell connections.")};
        case HiddenUniqueRectangle:
            return {.title = getLocalizedTechniqueName(technique),
                    .what_it_is = core::loc("A deadly rectangle pattern where one or more corners have the UR values "
        ...
        case AvoidableRectangle:
            return {
                .title = getLocalizedTechniqueName(technique),
                .what_it_is = core::loc("Like Unique Rectangle but using the distinction between given clues and "
                ...
                .what_to_look_for =
                    core::loc("Find rectangles where three corners are solver-placed (not givens) with two values.")};
        case ALSXYWing:
            return {.title = getLocalizedTechniqueName(technique),
                    .what_it_is = core::loc("Three non-overlapping Almost Locked Sets linked by restricted commons: "
                    ...
                    .what_to_look_for =
                        core::loc("Find three ALSs forming a chain with two restricted common candidates.")};
        case DeathBlossom:
            return {.title = getLocalizedTechniqueName(technique),
                    .what_it_is = core::loc("A stem cell with 2-3 candidates, each linked to a petal ALS via "
                    ...
                    .what_to_look_for =
                        core::loc("Find a cell whose candidates each connect to an ALS via restricted common.")};
        case VWXYZWing:
            return {.title = getLocalizedTechniqueName(technique),
                    .what_it_is = core::loc(
        ...
        case FrankenFish:
            return {
                ...
                .what_to_look_for = core::loc("Look for fish patterns that include boxes as base or cover sets.")};
        case GroupedXCycles:
            return {.title = getLocalizedTechniqueName(technique),
                    .what_it_is =
                        core::loc("Extends X-Cycles by allowing grouped nodes: 2-3 cells in the same box on the same "
                                  "row or column that all have a candidate digit. Same Type 1/2/3 rules apply."),
                    .what_to_look_for =
                        core::loc("Build X-Cycle chains using grouped box nodes alongside individual cells.")};
        case SashimiXWing:
            return {.title = getLocalizedTechniqueName(technique),
                    .what_it_is = core::loc(
                    ...
                    .what_to_look_for =
                        core::loc("Look for an X-Wing-like pattern where one row is incomplete — it only has the "
                                  "candidate in one of the two expected columns, plus an extra fin cell.")};
        case SashimiSwordfish:
            return {.title = getLocalizedTechniqueName(technique),
                    .what_it_is =
                        core::loc("A 3-row fish pattern where at least one base row has fewer candidate positions than "
        ...
        case SashimiJellyfish:
            return {.title = getLocalizedTechniqueName(technique),
                    .what_it_is =
                        core::loc("A 4-row fish pattern where at least one base row has fewer candidate positions than "
        ...
        case UnitForcingChain:
            return {.title = getLocalizedTechniqueName(technique),
                    .what_it_is = core::loc(
                        "For a digit in a unit with 2-3 positions, assume the digit goes in each position and "
                        "propagate. If all branches lead to the same conclusion, that conclusion is true."),
                    .what_to_look_for =
                        core::loc("Find a unit where a digit appears in few cells, then try each placement.")};
        case RegionForcingChain:
            return {.title = getLocalizedTechniqueName(technique),
                    .what_it_is =
                        core::loc("For a digit in a box with 2-3 positions, assume the digit goes in each position and "
                                  "propagate. If all branches lead to the same conclusion, that conclusion is true."),
                    .what_to_look_for =
                        core::loc("Find a box where a digit appears in few cells, then try each placement.")};
        case MutantFish:
            return {.title = getLocalizedTechniqueName(technique),
                    .what_it_is =
                        core::loc("A fish pattern where BOTH base and cover sets freely mix rows, columns, and boxes. "
        ...
        case KrakenFish:
            return {
                .title = getLocalizedTechniqueName(technique),
                .what_it_is =
                    core::loc("Extends finned fish by using chain propagation to verify eliminations outside the fin's "
                ...
                .what_to_look_for = core::loc("Find a finned fish pattern, then check if chain propagation from the "
                                              "fin cell eliminates the digit from cells outside the fin's box.")};
        case ALSChain:
            return {.title = getLocalizedTechniqueName(technique),
                    .what_it_is = core::loc("A generalized chain of 4-6 Almost Locked Sets linked by distinct "
                    ...
                    .what_to_look_for =
                        core::loc("Find a chain of ALSs where each adjacent pair shares a restricted common.")};
        case UniqueLoop:
            return {
                .title = getLocalizedTechniqueName(technique),
                .what_it_is =
                    core::loc("A deadly pattern where 4-6 cells form a loop, each consecutive pair sharing a unit "
                ...
                .what_to_look_for = core::loc("Find a loop of 4-6 cells across at least 2 boxes where each cell has "
        ...
        case JuniorExocet:
            return {.title = getLocalizedTechniqueName(technique),
                    .what_it_is = core::loc("A base pair of cells in a box whose candidates must appear in specific "
                    ...
                    .what_to_look_for =
                        core::loc("Find a base pair in a box with target cells in aligned boxes along cross-lines.")};
        case ContinuousNiceLoop:
            return {.title = getLocalizedTechniqueName(technique),
                    .what_it_is =
                        core::loc("A continuous alternating inference chain (AIC) that forms a complete loop. Every "
        ...
        case GroupedNiceLoop:
            return {.title = getLocalizedTechniqueName(technique),
                    ...
                    .what_to_look_for =
                        core::loc("Build AIC chains using grouped box nodes alongside individual cells. Look for "
                                  "discontinuous Type 2 chains where both endpoints assert the same digit.")};
        case Backtracking:
            return {.title = getLocalizedTechniqueName(technique),
                    .what_it_is = core::loc("A brute-force trial-and-error method. Not a logical technique — used as a "
                                            "fallback when no logical strategy can make progress."),
                    .what_to_look_for = core::loc("This technique is not used in training exercises.")};
    }
    return {.title = getLocalizedTechniqueName(technique),
            .what_it_is = core::loc("Unknown technique."),
            .what_to_look_for = core::loc("No identification tips available.")};
}
