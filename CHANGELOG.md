...
- **Saves, auto-saves, and session history are now written atomically.** All three wrote directly over their target file, so a crash or power loss mid-write left a truncated, unloadable file — and since auto-save is the crash-recovery path, an interrupted auto-save could destroy the very game state it exists to protect. Writes now go to a `<file>.tmp`, are `fsync`'d to durable storage, then atomically `rename`d into place, so a reader always sees either the previous complete file or the new complete one. The shared writer also adds the `fsync` step that the existing statistics atomic-write was missing. (#25)
