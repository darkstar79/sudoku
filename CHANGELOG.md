...
- **Kraken Fish steps no longer carry finned-fish eliminations under the wrong technique label.** `tryFinnedPattern` was bundling in-fin-box finned-fish eliminations (`free_elims`) with chain-verified Kraken eliminations into a single SolveStep tagged as Kraken Fish. Because FinnedXWing/Swordfish/Jellyfish run earlier in the strategy chain and already catch those in-box eliminations, the bundled output (1) inflated Kraken's apparent difficulty contribution, (2) made the step's `position` point at a non-Kraken cell, and (3) confused the technique-correctness harness. The step now reports only the chain-verified Kraken eliminations; the in-fin-box ones are left to the finned strategies. The step's `explanation_data.region_type` was also flipped to the *cover* axis (where eliminations sit), not the base axis — so the UI highlight now draws on the correct lines. (#20)
