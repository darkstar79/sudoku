...
// is a reliable "resumed THIS puzzle (not a new game)" discriminator.
const BoardData kPuzzle = {{0, 3, 0, 6, 7, 8, 9, 1, 2}, {6, 0, 2, 1, 9, 5, 3, 4, 8}, {0, 9, 8, 3, 4, 2, 5, 6, 7},
...
TEST_CASE("GameViewModel - Auto-save resume on restart",
    ...
    SECTION("AC#7: round-trip through the real SaveManager auto-save path resumes") {
        ...
        fixture.view_model->enterNote(*empty, 5);  // progress recorded; digits unchanged
        ...
        REQUIRE(loaded->is_auto_save);

        fixture.view_model->restoreGameState(*loaded);

        const auto& state = fixture.view_model->gameState.get();
        REQUIRE(state.extractGivenNumbers() == pre_givens);  // same puzzle reopened
        REQUIRE(state.getNotes(*empty).contains(5));         // pencil mark survived the round-trip
    }
}
